...
All that _every 2 weeks_, not bad!
...
A lot of the heavy lifting is done by a service called [Buttondown](https://buttondown.email) which is an elegant and advanced tool for creating newsletters and integrating it with a website. A lot of the _little_ things like setting a custom domain to send from (you’ll receive the Nevuletter from `news@nevulo.xyz`), hosting a custom site for people to subscribe, and a rich API to send information and set up a custom subscription form for my website.
